...
def plot_util(type_, y, title, f1,f2, resid):
    ...
    if type_ == 'scatter':
        ...
        #cbar.tick_params(labelsize=18, rotation=90, labelpad=30, weight='bold', fontsize=18)
        for t in cbar.ax.get_yticklabels():
        	t.set_fontsize(20)
        	t.set_fontweight('bold')

        ax.set_xlabel('Time (ns)', weight='bold', fontsize=20)
        ax.set_ylabel('Residue i', weight='bold', fontsize=20)
        xticks=np.arange(0, 501, 100)
        #xtick_labels = [f'{int(label)}' for label in xticks]
        #plt.xticks(xticks)
        #plt.xticks(weight='bold', fontsize=20, rotation=0)
        ax.set_xticks(xticks)
        ax.set_xticklabels([str(i) for i in xticks], weight='bold', fontsize=20, rotation=0)
        plt.yticks(weight='bold',  fontsize=20, rotation=0)
        plt.tick_params(width=2)
        lp = {'weight':'bold'}
        #plt.legend(fontsize=14, prop=lp, loc='upper right', frameon=False)
