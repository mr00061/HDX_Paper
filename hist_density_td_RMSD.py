...
def plot_density(y1,y2,y3,f1,t):
    ...
    X3=df3['C']
    sns.kdeplot(X1, label='r1', linewidth=3.5, color='black', cut=0)
    sns.kdeplot(X2, label='r2', linewidth=3.5, color='blue',cut=0)
    sns.kdeplot(X3, label='r3', linewidth=3.5, color='brown',cut=0)
...
def plot_hist(y1, y2, y3, y4,f2,t):
    df1=pd.DataFrame(y1, columns=list('A'))
    X1=df1['A']
    df2=pd.DataFrame(y2, columns=list('B'))
    X2=df2['B']
    ...
    #center=(bin_boundary[:-1]+bin_boundary[1:])/2
    #plt.bar(center, bin_height,width=width, lw=4, zorder=1)
    #df['A'].plot.kde(zorder=2, color='r')
    ax=sns.histplot(X1, bins=500 ,linewidth=2, label='AB',  color='black', stat='density', fill=True)
    ax=sns.histplot(X2, bins=500 ,linewidth=2, label='CD',  color='blue', stat='density', fill=True)
    ax=sns.histplot(X3, bins=500 ,linewidth=2, label='AC',  color='red', stat='density', fill=True)
    ax=sns.histplot(X4, bins=500 ,linewidth=2, label='BD',  color='green', stat='density', fill=True)
    #ax.set('%Contact /5Angstorm')

    plt.xlabel('%Contact /(5 Angstorm)', weight='bold', fontsize=12)
    plt.ylabel('Frequency', weight='bold', fontsize=12)
    plt.xticks(weight='bold')
    plt.yticks(weight='bold')
    plt.title(t, weight='bold', fontsize=12, loc='left')
    #rc('box', linewidth=2)
    plt.tick_params(width=2)
    #plt.axhline(linewidth=2)
    #mpl.rcParams['axes.linewidth'] =1
...
def plot_td(x1,y1,x2,y2,x3,y3,f3,t):
    fig, ax = plt.subplots(figsize=(7,6))
    plt.plot(x1,y1, label='r1', linewidth=3.0, color='black')
    plt.plot(x2,y2, label='r2', linewidth=3.0, color='blue')
    plt.plot(x3,y3, label='r3', linewidth=3.0, color='brown')
